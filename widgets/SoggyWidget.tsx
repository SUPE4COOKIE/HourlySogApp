...
import { FlexWidget, ImageWidget, requestWidgetUpdate } from 'react-native-android-widget';
import { storage, getIsDebugEnabled } from "@/storage/mmkv";
...
function getSyncRandomness(min: number, max: number): number {
	...
	const syncRdm = Math.floor(rng() * (max - min + 1)) + min;
	console.log(`Generated sync random number: ${syncRdm} (seeded with rounded time: ${rounded})`);
	return syncRdm;
}
...
function getRandomSoggyImage(): string | null {
	const keys = storage.getAllKeys().sort(); // if patch works maybe sort the database directly
	if (keys.length === 0) {
		return null;
	}
